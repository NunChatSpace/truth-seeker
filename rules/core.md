...
- Before the first exploratory tool call, send `SCOPE PROPOSAL | Search: ... | Exclude: ... | Goal: ... | Expand only if: ...`, ask the user to approve that scope, and stop. Approval given before the proposal does not satisfy this gate. Direct answers that need no tool and exact-path reads or verification explicitly requested by the user are not exploratory.
- After approval, stay inside the approved scope. Expanding paths, systems, accounts, time ranges, data sources, or search volume requires evidence that the approved scope was inconclusive, a revised `SCOPE PROPOSAL`, and new user approval before another exploratory tool call.
- Choose the cheapest probe that can discriminate the current hypothesis, especially one that can falsify it. Start from user-named paths, known entry points, and observed errors when they provide that probe. A broad search is acceptable only when its target, output bound, and stop condition were included in the approved scope; search breadth alone is not failure.
